...
makeCorePheatmaps <- function(TPMs_tidy, displayNums=T, mainTitle="heatmap", w=6.5, h=5) {
  ...
  thePheatmap <- pheatmap(cormat, annotation_col=anno, display_numbers = displayNums, main=mainTitle, breaks=seq(0.7,1,by=0.01), width=w, height=h, cex=0.5, show_colnames = F, treeheight_col = 10, treeheight_row = 10, color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdYlBu")))(30))
}
...
nanoVsLi_pheatmap <- makeCorePheatmaps(nanoLi_TPMs_tidy_filt_newSamps, mainTitle="method correlations", displayNums = F)
...
HEK_figure <-  (CDS_riboProf_colored | rpkm_byRegion_plot+scale_x_discrete(labels=HEK_samp_labels)) / (psite_freqHeat + scale_y_discrete(labels=HEK_samp_labels)+theme(legend.position = "top") | psiteFreq +scale_x_discrete(labels=HEK_samp_labels)) / as.ggplot(nanoVsLi_pheatmap)+plot_layout( heights=c(2,2,5))+plot_annotation(tag_levels = list(c("A","B","C","D","E"))) & theme(plot.tag=element_text(size=12, face="bold"))+theme(legend.key.size=unit(0.5,"line"))
