...
TPMs_filtered <- filterRaw(FC_tidy=featureCounts_tidy,threshold=20, frac_samples=5/10, experimentTypes = experimentTypes, TPMs=TPMs)
...
makeCorePheatmaps <- function(TPMs_tidy, displayNums=T, mainTitle="heatmap", w=6.5, h=5) {
  ...
  cormat <- cor(log10(dplyr::select(TPMs_mat, -Geneid)+1), method="spearman")
  ...
  thePheatmap <- pheatmap(cormat, annotation_col=anno, display_numbers = displayNums, main=mainTitle, breaks=seq(0.5,1,by=0.01), width=w, height=h, cex=0.5, show_colnames = F, treeheight_col = 10, treeheight_row = 10, color = colorRampPalette(rev(brewer.pal(n = 7, name =
  "RdYlBu")))(50))
}

CPN_single_hemi_corpHeat <-makeCorePheatmaps(TPMs_filtered_tidy, displayNums = F, w=6.5,h=5)
